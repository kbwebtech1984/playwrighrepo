...
- generic [active] [ref=e1]:
  ...
  - insertion [ref=e73]:
    - generic [ref=e76]:
      ...
      - link "Computer Security" [ref=e79] [cursor=pointer]
      - link "TV & Video" [ref=e84] [cursor=pointer]
      - link "Dictionaries & Encyclopedias" [ref=e89] [cursor=pointer]
```
...
```ts
  ...
  10 |     const data=xlsx.utils.sheet_to_json();
  11 | 
> 12 |     for(i=0;i<data.length;i++)
     |          ^ ReferenceError: i is not defined
  13 |     {
  14 |         await page.getByPlaceholder('Email Address').fill(data[i].username);
  15 |         await page.getByPlaceholder('Password').fill(data[i].password);
  16 |     }
  17 | })
```